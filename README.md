...
- HTTP 冒烟（对运行中的服务）：
  ...
  - 幂等重放：同操作同内容（含首次发布瞬间的并发同参调用）返回首次结果，
    异参复用报 `OPERATION_CONFLICT`
  - 同参并发合并：多个工程工作站以完全相同的操作标识、所见修订、区间与内容
    并发补发时只实际发布一次（1 个原始 201 + 其余带
    `X-Idempotent-Replay: true` 的同一响应体），后续重试保持该结果，head
    只前进一次，最终区间快照一致
